...
if [ "$ROOT_PIDS" -gt 0 ]; then
    echo "Process is running as root, will need sudo for kill"
    NEED_SUDO=true
else
    NEED_SUDO=false
fi
...
for i in {1..10}; do
    ...
    if [ -z "$STILL_RUNNING" ]; then
        ...
        echo "Port 5000 may still be in TIME_WAIT state (will clear in ~30-60 seconds)"
        exit 0
    fi
done

# Force kill if still running
echo "Process(es) still running after 5s, forcing kill..."
PIDS=$(ps aux | grep "[p]ython.*app_web.py" | awk '{print $2}')
for PID in $PIDS; do
